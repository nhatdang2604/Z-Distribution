...
func (h *GetHandler) Handle(concensusPath string) (int32, *zk.Stat, error) {
	...
	if err != nil {
		return 0, zkStat, fmt.Errorf("node id=%v had error on getting concensus value: %v", nodeId, err)
	}

	var consensusVal int32
	_, err = fmt.Sscanf(string(data), "%d", &consensusVal)
	if err != nil {
		return 0, zkStat, fmt.Errorf("node id=%v had error on getting consensus value with parsing: %v", nodeId, err)
	}
